...
def RVG_stent_description(
    ...
    if len(segments) == 3:
        if RVG_ostium.stent and RVG_proximal.stent and RVG_distal.stent:
            return (
                f"Presence d'un stent long au niveau de la rétro ventriculaire gauche"
            )

    if len(segments) == 2:
        if RVG_proximal.stent and RVG_ostium.stent:
            return f"Presence d'un stent au niveau de la rétro ventriculaire gauche ostio-proximale"
        if RVG_proximal.stent and RVG_distal.stent:
            return f"Presence d'un stent au niveau de la rétro ventriculaire gauche"
...
def RVG_stent_No_lesion_generator(state: CoronarySegmentation):
    ...
    report = (
        f"presence de {len(stents)} stents au niveau de la rétro ventriculaire gauche. "
        if len(stents) > 1
        else ""
    )

    for stent in stents:
        report += RVG_stent_description(state, stent)
