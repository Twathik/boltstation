...
def Cx_calcification_description(
    ...
    if len(segments) == 3:
        if (
            Cx1_ostium.calcification
            and Cx1_proximal.calcification
            and Cx1_distal.calcification
        ):

            return f"Presence d'une calcification diffuses au niveau de l'artère circonflexe proximale"

    if len(segments) == 2:
        if Cx1_proximal.calcification and Cx1_ostium.calcification:
            return f"Presence d'une calcification au niveau de l'artère circonflexe ostio-proximale"
        if Cx1_proximal.calcification and Cx1_distal.calcification:
            return f"Presence d'une calcification au niveau de l'artère circonflexe proximale"
        if Cx2_proximal.calcification and Cx2_distal.calcification:
            return f"Presence d'une calcification au niveau de l'artère circonflexe moyenne"
        if Cx3_proximal.calcification and Cx3_distal.calcification:
            return f"Presence d'une calcification au niveau de l'artère circonflexe distale"
    if len(segments) == 1:
...
def Cx_calcification_children_parser(
    state: CoronarySegmentation, segments: List[CoronarySegmentDescription]
):
    ...
    for sgt in segments:
        if (
            ...
            and not state.Marg1_ostium.lesion.calcification == True
        ):
            bifurcations.append(segments_mapping("Marg1_ostium"))
        if (
            sgt.segment_name == "Cx2_proximal"
            and not state.Marg2_ostium.lesion.calcification == True
        ):
            bifurcations.append(segments_mapping("Marg2_ostium"))
        if (
            sgt.segment_name == "Cx2_distal"
            and not state.IVPL_ostium.lesion.calcification == True
        ):
...
def Cx_calcification_No_stenosis_generator(state: CoronarySegmentation):
    ...
    report = (
        f"presence de {len(lesions)} calcification au niveau de l'artère circonflexe. "
        if len(lesions) > 1
        else ""
    )

    for lesion in lesions:
        report += Cx_calcification_description(state, lesion)
        report += Cx_calcification_children_parser(state, lesion)
